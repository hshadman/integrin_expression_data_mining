...
dataExpr0=read.csv("01_voomExpr_PRAD.csv");
head(dataExpr0[1:6])
```
...
plot(sampleTree,
     ...
     xlab = "",
     cex = 0.6);
abline(h = 90, col = "red"); #Cut tree at height 100.
```
```{r}
clust = cutreeStatic(sampleTree,
                     cutHeight = 90,
minSize = 10);
table(clust) #If cutting tree at height 90, 17 samples are deemed as outliers
```
...
```{r}
dataTraitALL = read.csv("00_ClinTraits_PRAD.csv");
dataTraitALL
```
